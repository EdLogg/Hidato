...
void DrawPuzzle(HWND hWnd, HDC hdc)
{
	...
	{
	case STATE_FORMAT:
		DrawText(hdc, "Deselect invalid areas via the mouse.\nPress Return to enter numbers.", -1, &TextRect, DT_CENTER | DT_VCENTER);
		break;
	case STATE_ENTER:
		DrawText(hdc, "Enter puzzle values.  Use the mouse or arrow keys to select entry.\nPress Return to solve puzzle.", -1, &TextRect, DT_CENTER | DT_VCENTER);
		break;
	case STATE_SOLVE:
		DrawText(hdc, "Select Alt+Space to solve the puzzle or press Space to make a single step.\nTo manually solve: select a tile, enter the value, then select elsewhere.", -1, &TextRect, DT_CENTER | DT_VCENTER);
		break;
	case STATE_DONE:
	...
	for (int y = 0; y < s.puzzleHeight; y++)
	{
		for (int x = 0; x < s.puzzleWidth; x++)
		{
			...
			{
				if (s.puzzle[y][x].val == 1
				|| s.puzzle[y][x].val == s.puzzleMax)
					SelectObject(hdcMem, hBoardEnd);
				else if (s.puzzle[y][x].added				// just added via the step function
				||  (y == s.cursorY && x == s.cursorX))		// we are manually changing this tile
					SelectObject(hdcMem, hBoardOn);
				else if (s.puzzle[y][x].val >= 0)			
					SelectObject(hdcMem, hBoard);
				else
					SelectObject(hdcMem, hBoardOff);
			}
